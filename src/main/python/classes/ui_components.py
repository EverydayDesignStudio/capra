...
class UILabelTop(UILabel):
    def __init__(self, window, size: int, margin: int, text: str, alignment):
        UILabel.__init__(self, window)
        ...
        self.setFont(QFont('Atlas Grotesk', size, 400))
...
class UIUnderlay(UILabel):
    def __init__(self, window, appContext):
        ...
        img = appContext.get_resource('underlay@1x.png')
        pixmap = QPixmap(img)
        self.setPixmap(pixmap)
...
class UIUnderlayBottom(UILabel):
    def __init__(self, window, appContext):
        ...
        img = appContext.get_resource('underlay-bottom@2x.png')
        pixmap = QPixmap(img)
        self.setPixmap(pixmap)
...
class UIHelpMenu(QWidget):
    ...
    def __init__(self, window: QMainWindow, appContext):
        super().__init__(window)
        ...
        img = appContext.get_resource('help-menu@2x.png')
        helpImage = UIImage(img)
...
class UIScope(UIWindowWidget):
    ...
    def __init__(self, window: QMainWindow, appContext):
        ...
        img = self.context.get_resource('scope-hikes@2x.png')
        self.scopeImage = UIImage(img)
        ...
        self._status_size = 1  # 0 - small; 1 - large
    ...
    def setScopeHikes(self):
        if self._status_size:  # 1
            img = self.context.get_resource('scope-hikes@2x.png')
            self.scopeImage.update_image(img)
        else:
            img = self.context.get_resource('scope-hikes@2x.png')
            self.scopeImage.update_image(img)

    def setScopeArchive(self):
        if self._status_size:
            img = self.context.get_resource('scope-archive@2x.png')
            self.scopeImage.update_image(img)
        else:
            img = self.context.get_resource('scope-archive@2x.png')
            self.scopeImage.update_image(img)

    # TODO -- the size doesn't seem to be changing
    def setSizeSmall(self):
        self._status_size = 0  # small
        if self._status_scope:  # 1 - Hikes
            self.setScopeHikes()
        else:
            self.setScopeArchive()

    # TODO
    def setSizeLarge(self):
        self._status_size = 1  # large
        if self._status_scope:  # 1 - Hikes
            self.setScopeHikes()
        else:
            self.setScopeArchive()
...
class AltitudeGraph(UIWidget):
    ...
    def __init__(self, appContext, isAltMode: bool, altitudeList: list, percent: float, currentAlt: float) -> None:
        super().__init__()
        ...
        img = appContext.get_resource('indicator-selected.png')
        self.indicatorSelected = QImage(img)
...
class ColorBar(UIWidget):
    ...
    def __init__(self, appContext, isColorMode: bool, colorList: list, percent: float, indicatorColor: QColor) -> None:
        super().__init__()
...
class TimeBar(UIWidget):
    ...
    def __init__(self, appContext, isTimeMode: bool, bgcolor, colorBarSize: int, percent: float) -> None:
        super().__init__()
        ...
        img1 = appContext.get_resource('indicator-time.png')
        img2 = appContext.get_resource('indicator-time-selected.png')
        self.indicator = QImage(img1)
        self.indicatorSelected = QImage(img2)
